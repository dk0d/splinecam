...
def get_region_statistics(polys):
    ...
    # avg_verts = (
    #     torch.stack(
    #         [torch.tensor(each.shape[0] - 1) for each in polys]
    #     )  # -1 because of first vert being repeated
    #     .type(torch.float32)
    #     .mean()
    # )

    return stats, torch.stack(
        [
            vol_m,
            vol_std,
...
def verify_collinear(v_new, v1, v2, eps: float = 1e-7):
    ...
    return torch.allclose(l1, l2 + l3, rtol=0.0, atol=eps)


## TODO: make this jittable
def get_region_means(
    regions: list, dims: int, dtype: object = torch.float64, device: str = "cuda"
):
    """
...
@torch.jit.script
def get_square_slice_from_one_anchor(anchors, pad_dist: int = 1):  # , seed=None):
...
def get_square_slice_from_two_anchors(
    anchors, pad_dist: int = 1, seed: Optional[int] = None
):
...
@torch.jit.script
def get_square_slice_from_centroid(anchors, pad_dist: int = 1, seed: int = 0):
...
def get_nneigh_points(dataset, target_classes):
    ...
    data1 = (
        torch.from_numpy(dataset["data"][mask])
        .type(torch.float32)
        .cuda()
        .transpose(1, 3)
    )  # channel first
