...
class JITAdapter(torch.jit.ScriptModule):
    """
    ...
    def __init__(self, T, name="adapter", device="cuda", dtype=torch.float32):
        """
        T : projection mat.
        """
    ...
    def get_activation_pattern(self, x):
        return torch.ones(
            x.shape[0], self.output_shape, device=x.device, dtype=x.dtype
        )  ##self.dtype issue due to jit

    @torch.no_grad()
    def get_intersection_pattern(self, x):
        return torch.ones(x.shape[0], self.output_shape, device=x.device, dtype=x.dtype)
...
class WrapperLinear(torch.nn.Module):
    ...
    def get_weights(self, row_idx=None, dtype=torch.float64):
        ...
        if not hasattr(self, "Ab"):
            ...
            if self.has_bn:
                ...
                c = self.bn_gamma / (self.bn_rvar + self.bn_eps) ** 0.5
    ...
    def get_activation_pattern(self, x):
        ...
        if not self.has_act:
            ...
            q = (pre_act > 0) * 1.0 + (pre_act <= 0) * self.act_nslope
...
class ModelWrapper(object):
    ...
    def __init__(
        ...
        if model.training:
            raise ValueError(
                "Model should always be supplied in eval mode. Please do `model.eval()` before wrapping"
            )
        ...
        if T is not None:
            self.layers.append(
                JITAdapter(T=self.T, dtype=self.dtype, device=self.device)
            )
    ...
    def compose_layers(self, layers, layer_names):
        ...
        not_supported = [
            ...
            if type(each) not in SUPPORTED_MODULES + SUPPORTED_ACT
        ]

        if len(not_supported) > 0:
            raise NotImplementedError(
                f"Model contains modules {not_supported} which are currently not supported"
            )
        ...
        for layer, name in zip(layers, layer_names):
            ...
            if type(layer) == torch.nn.modules.linear.Linear:
                new_layers.append(
                    WrapperLinear(layer, device=self.device, dtype=self.dtype)
                )
                ...
                if current_shape is None:
                    raise ValueError(
                        "Please specify model input shape for conv networks"
                    )
            ...
            ## if act add to prev layer
                ...
                type(layer) == torch.nn.modules.activation.ReLU
                or type(layer) == torch.nn.modules.activation.LeakyReLU
            ):
                new_layers[-1].add_act(layer)

            ## if bn add to prev layer
            elif (
                type(layer) == torch.nn.modules.BatchNorm1d
                or type(layer) == torch.nn.modules.BatchNorm2d
            ):
    ...
    @torch.no_grad()
    def verify(self, seed=0, eps=1e-5) -> bool:
        """
        ...
        if self.T is not None:
            ...
            A = (
                torch.randn(1, *self.input_shape, device=self.device)
                .type(self.dtype)
                .reshape(1, -1)
            )
...
class WrapperConv2d(torch.nn.Module):
    """
    Wrapper for a torch.nn.Linear layer with batchnorm and activation function
    Module forward is in sequence Linear->BN->Activation
    ...
    def __init__(
        ...
        Follows order Linear->BN->Activation
        """
        super(WrapperConv2d, self).__init__()
        ...
        assert (
            self.layer.kernel_size[0] - 2 * self.layer.padding[0] == 1
        )  ## padding same
    ...
    def prepare_weights(self):
        ...
        identity = torch.eye(
            torch.prod(self.input_shape).type(torch.int64),
            device="cpu",
            dtype=torch.float32,
        ...
        if self.has_bn and self.layer.bias is None:
            ...
            def method(x):
                return c[None, :, None, None] * self.layer.forward(x)

        elif self.has_bn and self.layer.bias is not None:

            def method(x):
                return c[None, :, None, None] * (
                    self.layer.forward(x) - self.layer.bias[None, :, None, None]
                )

        elif not self.has_bn and self.layer.bias is not None:

            def method(x):
                return self.layer.forward(x) - self.layer.bias[None, :, None, None]
        ...
        if self.layer.bias is not None:
            ...
            b = torch.stack(
                [
                    torch.ones(self.output_shape[1], self.output_shape[2])
                    .type(torch.float32)
                    .to(self.device)
                    * bi
                    for bi in bias
...
class WrapperAvgPool2d(torch.nn.Module):
    """
    ...
    def __init__(
        ...
        Follows order Linear->BN->Activation
        """
        super(WrapperAvgPool2d, self).__init__()
    ...
    def get_activation_pattern(self, x):
        ...
        """
        return torch.ones(
            x.shape[0], torch.prod(self.output_shape), device=self.device
        ).type(self.dtype)
