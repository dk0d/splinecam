...
def vertex_order_along_line_batched(endpoint1, endpoint2, v):
    ...
    v:                   batchsize x N x 2
    """
    v = torch.cat([endpoint1, endpoint2, v], dim=1)
    ...
    if torch.all(endpoint_match):
        ...
        flip_idx = torch.where(torch.logical_not(endpoint_match))[0]
...
def vertex_order_along_line(endpoint1, endpoint2, v):
    ...
    v:                   N x 2
    """
    v = torch.cat([endpoint1, endpoint2, v], dim=0)
...
def add_line_to_graph(G, node_names, start, end, v, line_name="", layer_name=-1):
    """
    ...
    G.add_edge(start, node_names[0], layer=layer_name, hyp=line_name)
    ...
    G.add_edge(node_names[-1], end, layer=layer_name, hyp=line_name)
...
def create_poly_hyp_graph(
    ...
    v, flag = find_intersection_2D(poly_lines, poly_int_hyps, verify=verify)
...
def hyp2input(hyps, Abw):
    ...
    bias_inp = torch.bmm(hyps[..., :-1], Abw[..., -1:]) + hyps[..., -1:]
...
def cycles_list2vec(regions, repeat_first: bool = True):
    ...
    if repeat_first:
        for i in range(len(regions)):
            regions[i] = torch.vstack([regions[i], regions[i][:1]])
...
def create_hyp_combinations(hyps, hyp_idx, endpoints):
    ...
    q *= torch.logical_not(torch.eye(q.shape[0]).to(q.device))
    no_inter_idx = torch.where(q.sum(1) == 0)[0]
    q *= mask

    # get combination
    loc = torch.where(q)
    comb_idx = torch.stack([hyp_idx[loc[0]], hyp_idx[loc[1]]]).T
