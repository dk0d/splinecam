...
domain = sc.utils.get_square_slice_from_one_anchor(
    torch.randn(1, 3 * 32 * 32), pad_dist=2, seed=None
)
...
NN = sc.wrappers.model_wrapper(model, input_shape=(3, 32, 32), T=T, dtype=torch.float64)
...
plt.savefig(
    "../figures/cnn_visualize.jpg", transparent=True, bbox_inches=0, pad_inches=0
)
